...
import { UserItemTypes } from '@/lib/types'
const profilePage = async () => {
     const sessionData = await getServerSession(authOptions)
     const userData : UserItemTypes = await getUserById(sessionData?.user.id ?? "");

  return (
   <section className='w-full h-full flex flex-col'>
    <div className='mx-auto'>
        <Image src={sessionData?.user.image ??"/userIcon.png"} alt='UserProfile' width={200} height={200} className='w-20'/>
    </div>
